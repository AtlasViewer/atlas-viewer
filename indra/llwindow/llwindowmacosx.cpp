...
BOOL check_for_card(const char* RENDERER, const char* bad_card)
{
	if (!strnicmp(RENDERER, bad_card, strlen(bad_card)))
	{
		std::string buffer = llformat(
			"Your video card appears to be a %s, which Atlas does not support.\n"
			"\n"
			"Atlas requires a video card with 32 Mb of memory or more, as well as\n"
			...
			"\n"
			"You can try to run Atlas, but it will probably crash or run\n"
	...
	: LLWindow(NULL, fullscreen, flags)
{
	...
	if(createContext(x, y, width, height, 32, fullscreen, vsync_setting))
	{
		...
		{
			setupFailure(
				"Atlas is unable to run because your video card drivers\n"
